...
where `babashka.process` comes in.

What if we wanted to start a Clojure pREPL in our script, send commands to it
and read the evaluated output?  Using `clojure.java.shell/sh` for this has a few
drawbacks:

- We have no control over stopping the pREPL process.
- We can't see any output from pREPL process until it finishes.

Using `babashka.process` which leverages the `java.lang.ProcessBuilder` and
`java.lang.Process` classes gives us the extra power we need to overcome these
limitations.
...
<2> We read user input via `clojure.core/read-line`

<3> Here we binding `*out*` to the input writer so we can use `println` to write
to it.

<4> A reader that reads from the pREPL output. pREPL returns EDN and
`clojure.edn/read` needs a `PushbackReader`.

<5> Read the next EDN value.

<6> If it's not the end of the input, then we print the output using our custom
formatting.
