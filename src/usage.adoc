...
https://github.com/clojure/tools.deps[tools.deps] with babashka and allows
...
[source,clojure]
----
$ export BABASHKA_CLASSPATH=$(bb clojure -Spath -Sdeps '{:deps {clj-commons/fs {:mvn/version "1.6.307"}}}')
----
...
`bb uberjar` uses tools.build, see <<tools-build>>.

[[tools-build]]
=== tools.build

Babashka includes https://github.com/clojure/tools.build[tools.build]. Use
`clojure.tools.build.api` in a script to build a jar:
...
(b/jar {:class-dir "target/classes"
        :jar-file "target/foo.jar"
        :lib 'my/foo
        :version "0.1.0"
        :basis basis})
----

`b/javac` runs the `javac` from `JAVA_HOME` or the `PATH`. `b/compile-clj`
runs `java`.

If tools.build is on the classpath as a dependency, babashka loads that version
instead.
...
`.getAbsolutePath`)
* `babashka.config`: the absolute path to the `bb.edn` (or other
config) file
...
Use `bb clojure` like the `clojure` command:
...
`-M`, `-X` and `-T` start a `java` process. See <<deps-resolution>>.
