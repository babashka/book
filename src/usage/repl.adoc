...
To accept connections from other hosts, pass `0.0.0.0` as the host:
...
Use `bb repl --connect` to connect to an nREPL server from the terminal.
The command reads the port from `.nrepl-port`:
...
To connect to another server, pass `host:port`, a port or `unix://path` to `bb repl --connect`.

Pass an options map to `babashka.nrepl.server/start-server!` to configure the server:
...
- `:middleware`: a vector of nREPL middleware vars, see
