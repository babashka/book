...
To capture output as a string, pass the special value `:string` and get the `:out` key from the resulting map. In most cases, you probably want to `trim` away the trailing newline as well:
