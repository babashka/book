...
Write `:cmd` as a vector of pairs to keep the order of more than 8 commands:
...
Use `:cli` for other babashka.cli options of the task, such as `:epilog`:
...
For options with a function, such as `:error-fn`, set `:cli` to the symbol of a
var that holds the map:
...
A task with `:exec-fn` in `:depends` runs before the task. Its options also
parse for the task, and `--help` lists them under `Inherited options`:
...
The function of `compile` receives all parsed options. Set `:restrict true` in
its `:org.babashka/cli` metadata to pass it only the options it declares.

`bb jar --help` does not run `:depends`.
