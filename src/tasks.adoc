...
Use <<tasks-cli,task CLI options>> to add `--help` and shell completion.
...
Use `:exec-fn` or `:cmd` to configure a task with https://github.com/babashka/cli[babashka.cli].
Tasks with these keys support automatic `--help`, shell completion and subcommands.

==== :exec-fn

Use `:exec-fn` with a fully qualified symbol to refer to a function that handles the task:
...
The function receives the parsed options as a map. Declare the options in the
`:org.babashka/cli` metadata of the function:
...
`bb tasks` uses the function docstring as the task description.
If the task has a `:doc` entry, that entry overrides the docstring.

Use `:exec-args` to set default arguments for the task function:
...
Use `:cmd` to add subcommands to a task:
...
Add `:exec-fn` to the task map to handle `bb db` without a subcommand.

Use a vector of command-name and option-map pairs to preserve the order of subcommands:
...
Set `:cmd` to a fully qualified symbol to read the command tree from a var:
...
----
(def db-cmds
  {"migrate" {:exec-fn 'tasks/db-migrate}
   "seed"    {:exec-fn 'tasks/db-seed}})
----

==== :cli

Use `:cli` to set babashka.cli options for the task, such as `:epilog`:
...
For function-valued options such as `:error-fn`, use a var that holds the `:cli` map.
Set `:cli` to the var symbol:

[source,clojure]
----
{:tasks {db {:cli tasks/db-cli
             :cmd {"migrate" {:exec-fn tasks/db-migrate}}}}}
----

Set `:cli` in the `:tasks` map to apply options to every task with `:exec-fn` or `:cmd`.

==== :doc

Write `:doc` as a vector of lines for a multi-line description. `bb tasks`
...
Use `:depends` to run another task first.
The calling task also accepts options from dependencies with `:exec-fn`.
The `--help` output lists these options under `Inherited options`:
...
The `tasks/compile` function receives all parsed options.
Set `:restrict true` in its `:org.babashka/cli` metadata to limit the map to options that the function declares.

`bb jar --help` does not run dependency tasks.

==== Shell completion

Add this command after `compinit` in `~/.zshrc` to complete task names and options:
