...
Babashka tasks is unopinionated when it comes to command line arguments.
Arguments are available in the dynamic var `*command-line-args*`. You are free
to use parse them using the built-in `tools.cli` library, use
https://github.com/nubank/docopt.clj[docopt], convert them to EDN using `(map
edn/read-string *command-line-args*)` or do whatever to handle them.

The same dynamic var can be rebound to ensure functions see a different
set of arguments:
...
When runnning a task, babashka assembles a small program which defines vars
...
- Single quotes are accidentally supported in some places, but are better avoided:
  `{:task '(foo)}` does not work, but `{:task (quote (foo))` does work. When requiring namespaces, use the `:requires` feature in favor of doing it manually using `(require '[foo])`.
