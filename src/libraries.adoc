...
* `clojure.main`: `demunge`, `ex-triage`, `repl`, `repl-read`, `repl-requires`
...
* https://github.com/clojure/data.priority-map[`clojure.data.priority-map`]
...
* https://github.com/clojure/tools.build[`clojure.tools.build.api`], see <<tools-build>>
* https://github.com/nrepl/nrepl[`nrepl`]: `nrepl.core`, `nrepl.server`,
`nrepl.transport`, `nrepl.middleware`, `nrepl.misc`
...
If you want to use another, typically newer, version of a built-in library,
after declaring (e.g. `org.babashka/cli {:mvn/version "<latest-version>"}`) in
`bb.edn` or other means, use `require`'s `:reload`:
...
{:mvn/version "1.3.0"}}}`, resolves it and then adds to the babashka
classpath accordingly. See <<deps-resolution>>.

`add-deps` returns a sorted vector of the added libs, or `nil` if none were
added. It keeps the versions of libs already on the classpath.
...
Optionally, `add-deps` takes a second arg with options:

- `:aliases`: a vector of alias keywords to resolve with.
- `:force`: when `true`, resolve again instead of using the cached classpath.
- `:env`: a map that replaces the environment for resolving.
- `:extra-env`: a map of environment variables to add.
...
`nil` and prints to `*out*` for commands like `-Stree`, and `-Spath`. These do
not need `java`. For `-M`,
...
Use `tty?` to check if `:stdin`, `:stdout` or `:stderr` is a terminal:
...
NOTE: `babashka.ffi` is experimental.

The `babashka.ffi` library calls C functions in shared libraries. See the
https://github.com/babashka/ffi[ffi] repo and its
https://github.com/babashka/ffi/blob/main/doc/guide.md[guide] for how to use it.
