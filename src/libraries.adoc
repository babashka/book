...
Use `add-deps` to resolve dependencies and add them to the babashka classpath.
Pass a dependency map as the first argument:
...
`add-deps` returns a sorted vector of the added libraries, or `nil` if it adds none.
It keeps the versions of libraries already on the classpath.

Pass an optional map as the second argument to configure dependency resolution:

- `:aliases`: a vector of alias keywords to use for dependency resolution.
- `:force`: if `true`, resolve dependencies again instead of using the cached classpath.
- `:env`: a map that replaces the environment for dependency resolution.
...
Use `babashka.ffi` to call C functions in shared libraries.
See the https://github.com/babashka/ffi/blob/main/doc/guide.md[ffi guide] for examples.
