...
Babashka bundles `clojure.test`. To run tests you can use a script like the following:
...
(require '[clojure.test :as t]
         'your-namespace.with-tests)                 ;; <1>

(def test-results
  (t/run-tests 'your-namespace.with-tests))          ;; <2>

(def failures-and-errors
  (let [{:keys [:fail :error]} test-results]
    (+ fail error)))                                 ;; <3>

(System/exit failures-and-errors)                    ;; <4>
----

<1> Load `clojure.test` along with your own tests
<2> Run the tests and store the returned information
<3> Extract failures and errors
