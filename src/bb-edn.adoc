...
shell environment to resolve dependencies with `java`:
...
With the default (non-Java) dependency resolver, S3 repositories are not (yet) supported, so use the JVM resolver instead.
