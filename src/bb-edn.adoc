...
Babashka resolves and downloads dependencies without a Java installation by default.

Set `:deps-resolver :jvm` in `bb.edn` or `BABASHKA_DEPS_RESOLVER=jvm` in the
environment to resolve dependencies with `java`:
...
For `add-deps`, set `:deps-resolver` in the dependency map.

With the default dependency resolver:

- For S3 repositories, use `:deps-resolver :jvm`.
- Set `GIT_SSH_COMMAND` and `GITLIBS_COMMAND` in the environment of the babashka process.
The resolver ignores these variables in the `:extra-env` map of `add-deps`.
