...
Since version 1.13.221, babashka resolves `:deps` and `babashka.deps/add-deps`
without `java`.
...
The environment variable `BABASHKA_DEPS_RESOLVER=jvm` does the same. The map
passed to `add-deps` also accepts `:deps-resolver`.

Differences with `:deps-resolver :jvm`:
